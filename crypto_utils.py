...
def encrypt_with_rsa_oaep(plaintext: bytes, public_pem: bytes) -> str:
    # OAEP com MGF1+SHA-256: padding probabilístico, seguro no modelo CCA.
    public_key = load_public_key(public_pem)
...
def derive_session_kek(seed_b64: str, counter: int) -> str:
    """
    Deriva a KEK desta sessão a partir do seed (longa duração)
    e do contador de login (incrementa a cada login bem sucedido).

    Cliente e servidor derivam-na independentemente — nenhum bit
    da KEK viaja pela rede. Estilo HOTP (RFC 4226), com SHA-256.

    KEK_sessão = HMAC-SHA256(seed, counter)
    """
